...
public:
	...
	TriSparseMatrix<ElemType> &operator =(TriSparseMatrix<ElemType> &copy);
	// 赋值运算符重载
	TriSparseMatrix<ElemType> operator +(const  TriSparseMatrix<ElemType> &add);
...
template <class ElemType>
void TriSparseMatrix<ElemType>::show()
{
	for (int i = 0; i < num; i++)
	{
...
//稀疏矩阵三元组加法实现
template <class ElemType>
TriSparseMatrix<ElemType> TriSparseMatrix<ElemType>::operator +(const  TriSparseMatrix<ElemType> &add)
{	
	if (rows != add.rows || cols != add.cols) //如果两个矩阵行列数不同 不能相加
	{
		TriSparseMatrix<ElemType> fail;
		return fail;
	}
		
	int newMaxSize;  //新的三元组的大小
	vector<int> newCols;     //新的三元组的非零元列数
	vector<int> newRows;     //新的三元组的非零元行数
	vector<ElemType> newNum;        //新的三元组每个非零元元素的大小
	int count = 0;
	for (int i = 0, j = 0; j < add.num || i < num;)
	{
		if (add.triElems[j].row < triElems[i].row)
		{
			newCols.push_back(add.triElems[j].col);
			newRows.push_back(add.triElems[j].row);
			newNum.push_back(add.triElems[j++].value);
			//cout << newRows[newRows.size() - 1] << '\t' << newCols[newCols.size() - 1] << '\t' << newNum[newNum.size() - 1] <<'\t'<<count++<< endl;
		}
		else if (add.triElems[j].row > triElems[i].row)
		{
			newCols.push_back(triElems[i].col);
			newRows.push_back(triElems[i].row);
			newNum.push_back(triElems[i++].value);
			//cout << newRows[newRows.size() - 1] << '\t' << newCols[newCols.size() - 1] << '\t' << newNum[newNum.size() - 1] << '\t' << count++ << endl;
		}
		else
		{
			if (triElems[i].col < add.triElems[j].col)
			{
				newCols.push_back(triElems[i].col);
				newRows.push_back(triElems[i].row);
				newNum.push_back(triElems[i++].value);
			...
			else if (triElems[i].col > add.triElems[j].col)
			{
				newCols.push_back(add.triElems[j].col);
				newRows.push_back(add.triElems[j].row);
				newNum.push_back(add.triElems[j++].value);
			...
			else
			{
				newCols.push_back(triElems[i].col);
				newRows.push_back(triElems[i].row);
				newNum.push_back(triElems[i++].value + add.triElems[j++].value);
				//cout << newRows[newRows.size() - 1] << '\t' << newCols[newCols.size() - 1] << '\t' << newNum[newNum.size() - 1] << '\t' << count++ << endl;
			}
		}	
	}
	newMaxSize = newRows.size();
	TriSparseMatrix<ElemType> newT(rows, cols);
	newT.num = newMaxSize;
	newT.triElems = new Triple<ElemType>[newT.num];
	for (int i = 0; i < newT.num; i++)
	{
		newT.triElems[i].row = newRows[i];
		newT.triElems[i].col = newCols[i];
		newT.triElems[i].value = newNum[i];
	}
	return newT;
...
template <class ElemType>
TriSparseMatrix<ElemType> &TriSparseMatrix<ElemType>::operator =(TriSparseMatrix<ElemType> &copy)
